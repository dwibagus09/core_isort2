			...
			<table id="attachment-table" class="table">
			  ...
			  <tbody>
				...
				<tr id="attachmentSpv<?php echo $attachmentSpv['attachment_id']; ?>">
				  <td><?php echo '<a href="/attachment/security/'.$attachmentSpv['filename'].'" target="_blank" class="attachment-file">'.$attachmentSpv['filename'].'</a>'; ?></td>
				...
				<tr id="attachment<?php echo $attachment['attachment_id']; ?>">
				  <td><?php echo '<a href="/attachment/security/'.$attachment['filename'].'" target="_blank" class="attachment-file">'.$attachment['filename'].'</a>'; ?></td>
			  ...
			  <div class="form-group">
				<div class="col-md-12" style="text-align:center;">
					<button id="previous" type="button" class="btn btn-success" style="width:250px;" onclick="javascript:$('body').mLoading();location.href='/default/security/editchiefreport/dt/<?php echo $this->security['report_date']; ?>'">Kembali ke halaman sebelumnya</button>
				</div>
			  </div>
			</form>
		  </div>
...
<!-- Attachment form -->
  <form action="/default/security/addchiefattachment" id="attachment-form" class="mfp-hide white-popup-block"  method="POST" enctype="multipart/form-data">
	...
	<input type="hidden" name="report_id" class="form-control col-md-7 col-xs-12" value="<?php echo $this->security[0]['chief_security_report_id']; ?>">
...
$(document).ready(function() {	
	...
	$(".remove-attachment").click(function() {
		...
		if(res == true)
		{
			location.href="/default/security/deletechiefattachmentbyid/id/"+this.dataset.id+"/report_id/<?php echo $this->security[0]['chief_security_report_id']; ?>";
		}
	});
