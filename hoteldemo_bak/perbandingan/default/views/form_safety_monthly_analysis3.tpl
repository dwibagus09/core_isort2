  ...
  <div class="detail-report">
	...
	<div class="row">
	  <div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel">
		  <div class="x_title">
			<h2>Safety Monthly Analysis <?php echo $this->monthYear; ?> - <?php echo $this->ident['initial']; ?></h2>
		  ...
		  <div class="x_content">
			...
			<?php /*
			<span class="section">DETAIL HASIL PENCAPAIAN</span>	
			<?php if(!empty($this->unsafe_condition)) { ?>
			<h5><strong>Hasil Temuan Di Lapangan</strong></h5>
			<table id="perlengkapan-table" class="table">
				...
				<?php
					$i = 1;
					foreach($this->unsafe_condition as $unsafe_condition) 
					{
				?>
				<tr>
					<td><?php echo $i; ?><input type="hidden" value="<?php echo $unsafe_condition['equipment_id']; ?>" name="unsafe_condition_id[]"></td>
					<td><?php echo $unsafe_condition['date']; ?></td>
					<td><?php echo $unsafe_condition['description']; ?></td>
					<td><textarea class="monthly_analysis_field" rows="1" name="unsafe_condition_langkah_antisipasi[]" required><?php echo $unsafe_condition_langkah_antisipasi['total']; ?></textarea></td>
				</tr>
				<?php $i++; } ?>
			</table>
			<?php } ?>

			<?php if(!empty($this->safety_induction)) { ?>
			<h5><strong>Pelaksanaan Pelatihan dan Safety Induction</strong></h5>
			<table id="perlengkapan-table" class="table">
				<tr>
					<th width="50">No</th>
					<th>Hari Tanggal</th>
					<th width="200">Jenis Pelatihan</th>
					<th width="200">Peserta</th>
					<th>Keterangan</th>
				</tr>
				<?php
					$i = 1;
					foreach($this->safety_induction as $safety_induction) 
					{
				?>
				<tr>
					<td><?php echo $i; ?><input type="hidden" value="<?php echo $safety_induction['equipment_id']; ?>" name="fire_accident_equipment_id[]"><input type="hidden" value="<?php echo $fire_accident_equipment_detail['equipment_detail_id']; ?>" name="fire_accident_equipment_detail_id[]"></td>
					<td><?php echo $safety_induction['equipment_name']; ?></td>
			...
			<h3>Area Tenant yang Rawan Kejadian dan Kecelakaan K3</h3>	
			<div class="table-dv">
			<table class="table">
			  <thead>
				<tr>
		...
		<?php if(!empty($this->list_tangkapan)) { ?>
			...
			<div class="table-dv">
				...
				<tbody>
					<?php foreach($this->pelaku_tertangkap_detail as $pelaku) {
			...
			<?php } ?>
			  ...
			  <span class="section">JENIS KECELAKAAN TERTINGGI</span>
			   <form class="form-horizontal form-label-left" action="/default/safety/savemonthlyanalysis3" method="POST" onSubmit="$('body').mLoading();">
				<input type="hidden" value="<?php echo $this->monthly_analysis_id; ?>" name="monthly_analysis_id">
				<div class="table-dv">
				...
				<tbody>
					...
					<tr>
						...
						<td><textarea name="rekomendasi[]" class="form-control col-md-7 col-xs-12" style="height:50px;" required><?php echo str_replace("<br>","&#13;",stripslashes($incident['rekomendasi'])); ?></textarea></td>
					</tr>
					<?php $i++; } } ?>
				</tbody>
				</table>
			  ...
			  <div class="form-group">
				<div class="col-md-12" style="text-align:center;">
				  <button id="previous" type="button" class="btn btn-success" style="width:250px;" onclick="javascript:$('body').mLoading();location.href='/default/safety/monthlyanalysis2/id/<?php echo $this->monthly_analysis_id; ?>'">Kembali ke halaman sebelumnya</button>
			...
			open: function() {
			  $.ajax({
					...
					$.ajax({
						...
						data: { kejadian_id : resp.kejadian_id, category_id : '3' }
					}).done(function(response) {
						$("#modus_id").empty();
					...
					var detail = "<br/><label>Location</label><br/>"+resp.location+"<br/><br/><label>Discussion</label><br/>"+resp.description+"<br/><br/><label>Issue Date</label><br/>"+resp.date_time+"<br/><br/>";
					...
					$("#kejadian_id").change(function() {
						...
						{			
							...
							$.ajax({
								url: "/default/issue/getmodusbykejadianid",
								data: { kejadian_id : $( this ).val(), category_id: '3'  }
							}).done(function(response) {
								$("#modus_id").empty();
								var object = $.parseJSON(response);
								$.each(object, function (item, value) {
	...
	$('#issue-form').on('submit', function(event){
		...
		$.ajax({
			...
			success: function() {
				...
				<?php } ?>
				location.href="/default/safety/monthlyanalysis3"+idparam+"?"+issue_id+"#issue"+issue_id;
			}
		});
	});
});
