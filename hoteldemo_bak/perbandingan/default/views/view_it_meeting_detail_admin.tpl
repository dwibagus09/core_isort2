	...
	<div class="row">
	  <div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel">
			...
			<form id="it-form" class="form-label-left" action="" method="POST">
				...
				<div class="x_content">
					...
					<thead>
						<tr>
						<th width="100" rowspan="2">PIC</th>
						<th rowspan="2">Projects / Issues</th>					  
						<th colspan="3">Date</th>
						<th colspan="2" width="350">Follow Up</th>
						...
						<th width="175"></th>						  
						<th width="175"><?php echo $this->itMeeting['tanggal']; ?></th>
						</tr>
					</thead>
					<?php
						if(!empty($this->topic))
					...
					?>
						...
						<td><textarea name="followup[]" class="form-control col-md-7 col-xs-12 followup-txtarea" style="height:70px;" placeholder="jangan lupa mengisi status Digital Office"><?php echo str_replace("<br>","&#13;",$topic['follow_up']); ?></textarea><input class="form-control col-md-7 col-xs-12" name="followup_id[]" type="hidden" value="<?php echo $topic['followup_id']; ?>"></td>
...
<form action="" id="target-date-form" class="mfp-hide white-popup-block">
	<input type="hidden" name="it_meeting_id" value="<?php echo $this->itMeeting['it_meeting_id']; ?>" />
...
$(document).ready(function() {
	...
	$('#target-date-form').on('submit', function(event){
		...
		$.ajax({
			...
			success: function(response) {
				...
				<?php if(!$this->allowDeleteITMeeting) { ?>
	...
	$('#it-form').on('submit', function(event){
		event.preventDefault(); 
		$('body').mLoading();
		$.ajax({
			url: '/default/it/savedetailadmin',
			type: 'POST',
			data: $(this).serialize(),
			success: function(response) {
				$("body").mLoading('hide');	
	...
	$('.remove-start-date').click(function() {
		...
		if(res == true)
		{
			...
			$('body').mLoading();
			$.ajax({
				url: "/default/it/deletestartdate",
	...
	$('.remove-target-date').click(function() {
		...
		if(res == true)
		{
			...
			$('body').mLoading();
			$.ajax({
				url: "/default/it/deletetargetdate",
				data: { id : this.dataset.id, topic_id : topicid }
			}).done(function(response) {
				$("#target-date"+topicid).html(response);
				$("body").mLoading('hide');	
