  ...
  <div class="detail-report">
	...
	<div class="row">
	  <div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel">
		  <div class="x_title">
			<h2>Parking &amp; Traffic Monthly Analysis <?php echo $this->monthYear; ?> - <?php echo $this->ident['initial']; ?></h2>
			<div class="clearfix"></div>
		  </div>
		  <div class="x_content">
			<span class="section">ANALISA</span>
			...
			<h3>Area Parkir yang rawan kejadian</h3>	
			<div class="table-dv">
			<table class="table">
			  <thead>
				<tr>
				  <th rowspan="2">Area Parkir</th>
			  ...
			  <tbody>
				...
				<?php } } ?>
					<tr>
						...
						<td><strong><?php echo ($this->urutan_total_all_issue_tenant+$this->urutan_total_all_issue_publik); ?></strong></td>
					</tr>
			  </tbody>
			</table>
			</div>
			...
			<form class="form-horizontal form-label-left" action="/default/parking/savemonthlyanalysis" method="POST" onSubmit="$('body').mLoading();">
			<?php if(!empty($this->incidents)) { ?>
			  <input type="hidden" value="<?php echo $this->monthly_analysis_id; ?>" name="monthly_analysis_id">
			  <span class="section">JENIS KEJADIAN YANG SERING TERJADI BERDASARKAN JUMLAH KASUS BULAN <?php echo strtoupper($this->monthYear); ?></span>			  
				<div class="table-dv">
			...
			open: function() {
			  $.ajax({
					...
					$.ajax({
						...
						data: { kejadian_id : resp.kejadian_id, category_id : '5' }
					}).done(function(response) {
						$("#modus_id").empty();
					...
					$("#kejadian_id").change(function() {
						...
						{			
							...
							$.ajax({
								url: "/default/issue/getmodusbykejadianid",
								data: { kejadian_id : $( this ).val(), category_id: '5'  }
							}).done(function(response) {
								$("#modus_id").empty();
								var object = $.parseJSON(response);
								$.each(object, function (item, value) {
	...
	$('#issue-form').on('submit', function(event){
		...
		$.ajax({
			...
			success: function() {
				...
				<?php } ?>
				location.href="/default/parking/addmonthlyanalysis"+idparam+"?"+issue_id+"#issue"+issue_id;
			}
		});
	});
});
