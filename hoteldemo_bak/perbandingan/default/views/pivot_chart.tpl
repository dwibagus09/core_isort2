...
<div id="pivot-chart">
	<div class="row">
	  <div class="col-md-12 col-sm-12 col-xs-12">
	  	<h2 class="pagetitle">Business Intelligence</h2>
		
		<div class="pivot-filter">
			<form id="pivot-filter-form" action="/default/pivot/index/c/<?php echo $this->cat_id; ?>"  method="post">
				...
				<div id="filter-fields">
					...
					<div class="pivot-filter-field">
						...
						<div class="field-value">
							<select id="year" name="year"><?php for($i = 2020; $i<=date("Y");$i++) { ?><option <?php if($this->year == $i) echo "selected"; ?> value="<?php echo $i; ?>"><?php echo $i; ?></option><?php } ?></select>
						</div>
					...
					<div class="pivot-filter-field">
						...
						<div class="field-value">
							<select name="issue_type[]" id="issue_type" multiple>		
								<?php foreach($this->issue_type as $issue_type) { ?>			
									<option value="<?php echo $issue_type['issue_type_id']; ?>" <?php if($this->sel_issue_type[$issue_type['issue_type_id']] == 1) echo "selected"; ?>><?php echo $issue_type['issue_type']; ?></option>
								<?php }?>
							</select>
						</div>
					</div>
					<div class="pivot-filter-field">
						...
						<div class="field-value">
							<select name="kejadian[]" id="kejadian" multiple>		
								<?php if(!empty($this->incident)) { foreach($this->incident as $kejadian) { ?>			
									<option value="<?php echo $kejadian['kejadian_id']; ?>" <?php if($this->incidents[$kejadian['kejadian_id']] == 1) echo "selected"; ?>><?php echo $kejadian['kejadian']; ?></option>
								<?php } } ?>
							</select>
						</div>
							...
							<div class="field-value">
								<select name="modus[]" id="modus" multiple style="min-width:100px;">					
									<?php if(!empty($this->modus)) { foreach($this->modus as $modus) { ?>			
										<option value="<?php echo $modus['modus_id']; ?>" <?php if($this->mods[$modus['modus_id']] == 1) echo "selected"; ?>><?php echo $modus['modus']; ?></option>
									<?php } } ?>
								</select>
							</div>
						</div>
		...
		<div id="pivot-chart-container">
			...
			<div id="total-each-issue-type" class="pivot-chart-small">
				<canvas id="totalEachIssueType"></canvas>
			</div>
			
			<div id="total-monthly-chart" class="pivot-chart-small">
				<canvas id="totalMonthlyChart"></canvas>
			</div>			
			
			<div id="total-day-chart" class="pivot-chart-small">
			...
			<div id="total-each-area" class="pivot-chart-small">
...
$(document).ready(function() {
	...
	$('#issue_type').on('change', function() {
		$.ajax({
			url: '/default/issue/getkejadianbyissuetypes',
			data: { issue_type_ids : $(this).val(),
			...
			$.each(resp, function (item, value) {
				$("#kejadian").append(new Option(value.kejadian, value.kejadian_id));
			});
	...
	$('#kejadian').on('change', function() {
		$.ajax({
			url: '/default/issue/getmodusbykejadianids',
			data: { kejadian_ids : $(this).val(),
					category_id : '<?php echo $this->cat_id; ?>'
			}
		}).done(function(response) {
			var resp = $.parseJSON(response);
			$("#modus").empty();
			$.each(resp, function (item, value) {
				$("#modus").append(new Option(value.modus, value.modus_id));
			});
	...
	$("#reset-chart").click(function() {
		location.href = '/default/pivot/index/c/<?php echo $this->cat_id; ?>';
	});	
	...
	$("#exportpdf").click(function() {
		...
		var totalEachIssueType = document.getElementById("totalEachIssueType");
		...
		$.ajax({
			...
			data: {
				...
				totalEachIssueType: totalEachIssueType.toDataURL("image/png"),
			...
			success: function(data) {
				if(window.innerWidth <= 800 && window.innerHeight <= 600) {
					location.href = '/default/pivot/exportpivottopdf/cd/'+data+'/pf/<?php echo $this->ident['initial']."_".$this->cat_id; ?>';
				} else {
					window.open("/default/pivot/exportpivottopdf/cd/"+data+"/pf/<?php echo $this->ident['initial']."_".$this->cat_id; ?>");
				}	
				$("body").mLoading('hide');	
			}
		});
	...
	<?php if(!empty($this->totalEachType)) { ?>
	var issuetypeName = new Array();
	var issuetypeValue = new Array();
	var issuetypeColor = new Array();

	<?php
		$it = 0;
	 	foreach($this->totalEachType as $tt) { 
			echo 'issuetypeName['.$it.'] = "'.$tt['issue_type'].'"; ';
			echo 'issuetypeValue['.$it.'] = '.$tt['total'].'; ';
			echo 'issuetypeColor['.$it.'] = chartColor['.$it.']; ';
			$it++;
	} ?>
	...
	var issuetypeConfig = {
		...
		data: {
			datasets: [{
				data: issuetypeValue,
				backgroundColor: issuetypeColor,
				label: ''
			}],
	...
	var issuetypeChart = document.getElementById('totalEachIssueType').getContext('2d');
	window.issuetypeDoughnut = new Chart(issuetypeChart, issuetypeConfig);
<?php }  else  {
		echo "$('#total-each-issue-type').hide();";
	} ?>
