	...
	<div class="row">
	  <div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel">
			...
			<form class="form-label-left" action="/default/it/savedetail" method="POST" onsubmit="$('body').mLoading();">
				...
				<div class="x_content">
					...
					<div class="item form-group">
						...
						<div class="col-md-6 col-sm-6 col-xs-12">
							<?php foreach($this->attendance as $attendance) { ?>
								<div class="attendance-name"><?php echo $attendance['name']; ?></div>
							<?php } ?>
						</div>
					</div>
					<br clear="all" />
					...
					<thead>
						<tr>
						<th width="150" rowspan="2">PIC</th>
						<th rowspan="2">Projects / Issues</th>					  
						<th colspan="3">Date</th>
						<th rowspan="2">Follow Up</th>
						<?php if($this->approveITMeeting != 1 && $this->itMeeting['approved'] != 1) { ?><th rowspan="2">Done</th><?php } ?>
						</tr>
						<tr>
					...
					?>
						...
						<?php if($this->approveITMeeting != 1 && $this->itMeeting['approved'] != 1) { ?><td align="center"><input type="checkbox" id="done_checkbox" name="done_by_pic[<?php echo $i; ?>]" value="1" <?php if($topic['done_by_pic'] == '1') echo "checked"; ?>><input class="form-control col-md-7 col-xs-12" name="topic_id[]" type="hidden" value="<?php echo $topic['topic_id']; ?>"></td><?php } ?>
						</tr>
						<?php
								$i++;
							}
					...
					<div class="form-group">
						...
						<?php if($this->approveITMeeting != 1 && $this->itMeeting['approved'] != 1) { ?><button id="send" type="submit" class="btn btn-success" style="width:250px;">Simpan</button><?php } ?>
						<?php if($this->approveITMeeting == 1 && $this->itMeeting['approved'] != 1) { ?><button id="approve" type="button" class="btn btn-success" style="width:250px;" >Approve</button><?php } ?>
						</div>
					</div>
				</div>
			</form>
...
$(document).ready(function() {
	...
	var box = $("#chat_div").chatbox({
		...
		messageSent : function(id, user, msg) {
			...
			myFormData.append('it_meeting_id', '<?php echo $this->itMeeting['it_meeting_id']; ?>');
		...
		boxManager: {
                ...
                addMsg: function(peer, msg, filename) {
					...
					if(filename !== '')
					{
						msg = msg + '<br><a href="<?php echo $this->baseUrl; ?>/images/it_meeting/comments_'+filename.substring(15, 21)+'/'+filename+'" target="_blank"><i class="fa fa-paperclip"></i> ' + filename + '</a>';
					}
                    $(msgElement).html(msg);
	...
	<?php if($this->approveITMeeting == 1 && $this->itMeeting['approved'] != 1) { ?>
		$('#approve').click(function() {
			var res = confirm("Are you sure you want to approve this MoM?");
			if(res == true)
			{
				$("body").mLoading();
				location.href="/default/it/approvemom/id/<?php echo $this->itMeeting['it_meeting_id']; ?>";
			}
		});
	<?php } ?>
