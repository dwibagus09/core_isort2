			...
			<form class="form-horizontal form-label-left" action="" method="POST">
			  ...
			  <div class="item form-group">
				...
				<div class="col-md-6 col-sm-6 col-xs-6" style="padding-top:4px; text-align:center;">
					<?php echo date("d", mktime(0, 0, 0, date("m")  , date("d")-1, date("Y"))); ?>
				</div>
			...
			<table id="attachment-table" class="table">
			  ...
			  <tbody>
				...
				<tr id="attachment<?php echo $attachment['attachment_id']; ?>">
				  <td><?php echo '<a href="'.$this->baseUrl.'/default/attachment/openattachment/c/3/f/'.$attachment['filename'].'" target="_blank" class="attachment-file">'.$attachment['description'].'</a>'; ?></td>
			  ...
			  <div class="form-group">
				<div class="col-md-12" style="text-align:center;">
				  <button id="previous" type="button" class="btn btn-success" style="width:250px;" onclick="javascript:$('body').mLoading();location.href='/default/safety/edit/id/<?php echo $this->safety['report_id']; ?>'">Kembali ke halaman sebelumnya</button>  <button id="next" type="button" class="btn btn-success" style="width:250px;" onclick="javascript:$('body').mLoading();location.href='/default/manpower/view/c/3'">Ke halaman berikutnya</button>
				</div>
			  </div>
			</form>
		  </div>
...
<!-- Attachment form -->
  <form action="/default/safety/addattachment" id="attachment-form" class="mfp-hide white-popup-block"  method="POST" enctype="multipart/form-data">
	...
	<input type="hidden" name="report_id" class="form-control col-md-7 col-xs-12" value="<?php echo $this->safety['report_id']; ?>">
...
$(document).ready(function() {	
	...
	$(".add-attachment").click(function() {
		...
		if(id > 0)
		{
			...
			$.ajax({
				url: "/default/safety/getattachmentbyid",
				data: { id : id }
				...
				{
					$("#attachment_file").html('<a href="/attachment/safety/'+obj.filename+'" target="_blank" >'+obj.filename+'</a>');
				}
			});	
		}
		else
	...
	$(".remove-attachment").click(function() {
		...
		if(res == true)
		{
			location.href="/default/safety/deleteattachmentbyid/id/"+this.dataset.id+"/report_id/<?php echo $this->safety['report_id']; ?>";
		}
	});
