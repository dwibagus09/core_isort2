	  ...
	  <div class="col-md-12 col-sm-12 col-xs-12">
		  ...
		  <table class="table table-striped">
			...
			?>
				...
				?>
				<tr>
				  <td class="date-column"><?php echo $housekeeping['day_date']; ?></th>
				  <td class="date-column"><?php echo $housekeeping['name']; ?></td>
				  <td>
						<div class="three-newest-comments" id="comment-<?php echo $housekeeping['housekeeping_report_id']; ?>">
						<?php if(!empty($housekeeping['comments'])) { 
							foreach($housekeeping['comments'] as $comment)
							{
				  ...
				  <td class="action-column">
					<?php if($this->showEditHousekeeping == 1 && $housekeeping['allowEdit'] == 1) {  ?><a href="/default/housekeeping/edit/id/<?php echo $housekeeping['housekeeping_report_id']; ?>" class="action-btn"><i class="fa fa-edit" ></i></a><?php } ?>
					<?php /* <a class="action-btn delete"><i class="fa fa-trash" data-id="<?php echo $housekeeping['housekeeping_report_id']; ?>" ></i></a> */ ?>
					<a href="/default/housekeeping/viewdetailreport/id/<?php echo $housekeeping['housekeeping_report_id']; ?>" class="action-btn"><img src="/images/view_report2.jpg" width="27" /></a>
					<a class="add-comment" href="#comment-form" data-id="<?php echo $housekeeping['housekeeping_report_id']; ?>"><img src="/images/comment_24x24.png" /></a>
				  </td>
				</tr>
				<?php
						$i++;
...
$(document).ready(function() {
	...
	$(".fa-trash").click(function() {
		...
		if(res == true)
		{
			location.href="/default/housekeeping/deletereport/id/"+this.dataset.id;
		}
	});
	...
	$('.add-comment').magnificPopup({
		...
		callbacks: {
			open: function() {
			  $.ajax({
					url: "/default/housekeeping/getcommentsbyreportid",
					data: { id : id }
			...
			close: function() {	
				...
				$.ajax({
                    url: "/default/housekeeping/updatecomments",
                    data: { 
                    ...
                    $.each( resp, function( idx, val ) {
                        $( "#comment-"+val['housekeeping_report_id'] ).html(val['comment']);
                    });
                });
			}
		}
	...
	/*setInterval(function(){ 
		$.ajax({
			url: "/default/housekeeping/getupdatedcomments",
