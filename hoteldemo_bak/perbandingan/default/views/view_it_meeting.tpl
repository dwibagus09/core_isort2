...
<link rel="stylesheet" href="/css/magnific-popup.css">
	...
	<div class="row">
	  <div class="col-md-12 col-sm-12 col-xs-12">
			<h2 class="pagetitle">IT Meeting MoM</h2>
			<div class="paging">
		  ...
		  <table class="table table-striped">
			  ...
			  <?php
				if(!empty($this->itMeeting))
				{
			?>
				<tbody>
				<?php
					$i = 1;
					foreach($this->itMeeting as $itMeeting) { 
				?>
				<tr>
				  <td class="date-column"><?php echo $itMeeting['day_date']; ?></th>
				  <td class="date-column"><?php echo $itMeeting['meeting_title']; ?></td>
				  <td>
						<div class="three-newest-comments" id="comment-<?php echo $itMeeting['it_meeting_id']; ?>">
						<?php if(!empty($itMeeting['comments'])) { 
							foreach($itMeeting['comments'] as $comment)
							{
								...
								if(!empty($comment['filename'])) echo '<a href="'.$this->baseUrl.'/images/it_meeting/comments_'.$cdate[0].$cdate[1].'/'.$comment['filename'].'" target="_blank"><i class="fa fa-paperclip"></i> '.$comment['filename'].'</a>';
				  ...
				  <td class="action-column">
					<?php if($itMeeting['approved'] != 1 && $this->showITMeetingAdmin == 1) { ?><a href="/default/it/edit/id/<?php echo $itMeeting['it_meeting_id']; ?>" class="action-btn"><i class="fa fa-edit" ></i></a><?php } ?>
					<a href="/default/it/viewdetail/id/<?php echo $itMeeting['it_meeting_id']; ?>" class="action-btn"><img src="/images/view_report2.jpg" width="24" /></a> <a href="/default/it/exporttopdf/id/<?php echo $itMeeting['it_meeting_id']; ?>" target="_blank"><img src="/images/newlogo_pdf.png" width="24"></a>
				  	<a class="add-comment" href="#comment-form" data-id="<?php echo $itMeeting['it_meeting_id']; ?>" style="float:none; padding-top:10px;"><img src="/images/comment_24x24.png" /></a>
				  </td>
				</tr>
				<?php
						$i++;
...
<!-- comment form -->
  <form action="" id="comment-form" class="mfp-hide white-popup-block" >
	<input type="hidden" name="it_meeting_id" id="it_meeting_id" />
...
$(document).ready(function() {
	...
	$('.add-comment').magnificPopup({
		...
		callbacks: {
			open: function() {
			  $.ajax({
					url: "/default/it/getcommentsbymeetingid",
					data: { id : id }
				}).done(function(response) {
					$("#it_meeting_id").val(id);
			...
			close: function() {	
				...
				$.ajax({
                    ...
                    $.each( resp, function( idx, val ) {
                        $( "#comment-"+val['it_meeting_id'] ).html(val['comment']);
                    });
                });
			}
		}
