  ...
  <div class="detail-report">
	...
	<div class="row">
	  <div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel">
		  <div class="x_title">
			<h2 class="pagetitle">Monthly Analytics <?php echo $this->monthYear; ?> - <?php echo $this->ident['initial']; ?></h2>
		  ...
		  <div class="x_content">
			...
			<table id="perlengkapan-table" class="table">
			  ...
			  <tbody>
				...
				<tr>
					...
					<td align="center"><a href="#edit-kaizen-form" class="action-btn" data-id="<?php echo $issue['issue_id']; ?>"><i class="fa fa-edit" ></i></a></td>
				</tr>
				<?php $i++; } ?>
			  </tbody>
			</table>
			...
			<form action="" id="edit-kaizen-form" class="mfp-hide white-popup-block" >
			...
			<form class="form-horizontal form-label-left" action="/default/security/savemonthlyanalysis" method="POST" onSubmit="$('body').mLoading();">
			  ...
			  <div class="form-group">
				<div class="col-md-12" style="text-align:center;">
				  <button id="send" type="submit" class="form-btn" style="width:200px;">Simpan</button>
				</div>
			  </div>
			</form>
		  </div>
...
$(document).ready(function() {
			...
			open: function() {
			  $.ajax({
					...
					$.ajax({
						...
						data: { kejadian_id : resp.kejadian_id, category_id : '1' }
					}).done(function(response) {
						$("#modus_id").empty();
					...
					$("#kejadian_id").change(function() {
						...
						{			
							...
							$.ajax({
								url: "/default/issue/getmodusbykejadianid",
								data: { kejadian_id : $( this ).val(), category_id: '1'  }
							}).done(function(response) {
								$("#modus_id").empty();
								var object = $.parseJSON(response);
								$.each(object, function (item, value) {
	...
	$('#edit-kaizen-form').on('submit', function(event){
		...
		$.ajax({
			...
			success: function() {
				...
				<?php } ?>
				location.href="/default/security/addmonthlyanalysis"+idparam+"?"+Math.random()+"#issue"+issue_id;
			}
		});
	});
