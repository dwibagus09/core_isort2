...
class SafetycomitteeController extends actionControllerBase
{
    public function dashboardAction() {
		...
		$logData['action'] = "View Safety Committee Dashboard";
		$logData['data'] = "View Safety Committee Dashboard";
	...
	public function addAction() {
		if($this->showAddSafetyComittee)
		{
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Add Safety Committee";
			$logData['data'] = "Add Safety Committee";
			$logsTable->insertLogs($logData);	

			$category = $this->loadModel('category');
			$this->view->categories = $category->getCategories();

			$this->view->title = "Add Safety Committee MoM";
	...
	public function editAction() {
		if($this->showAddSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Edit Safety Committee";
			...
			$this->view->title = "Edit Safety Committee MoM";
	...
	public function saveAction() {
		if($this->showAddSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Save Safety Committee";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function safetycomitteeform2Action() {
		if($this->showAddSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Open Form Safety Committee 2";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function savetopicAction() {
		if($this->showAddSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Save Safety Committee Topic";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function viewAction() {
		...
		$logsTable = $this->loadModel('logs');
		$logData['user_id'] = intval($this->ident['user_id']);
		$logData['action'] = "View Safety Committee MOM List";
		$logData['data'] = json_encode($params);
		$logsTable->insertLogs($logData);	
	...
	public function viewdetailAction() {
		if($this->showSafetyComittee == 1 || $this->showSafetyComitteeAdmin == 1)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "View Safety Committee MoM";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function gettopicbyidAction() {
		if($this->showAddSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Get Safety Committee Topic By Id";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function deletetopicAction() {
		if($this->showAddSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Delete Safety Committee Topic By Id";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function addtargetdateAction() {
		if($this->showAddSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Add Safety Committee Topic Target Date";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function addstartdateAction() {
		if($this->showAddSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Add Safety Committee Topic Start Date";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function savedetailadminAction() {
		if($this->showAddSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Save Safety Committee Detail Admin";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function savedetailAction() {
		if($this->showSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Save Safety Committee Detail";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function approvemomAction() {
		if($this->approveSafetyComittee)
		{
			...
			if(!empty($params['safety_comittee_id']))
			{
				...
				$logsTable = $this->loadModel('logs');
				$logData['user_id'] = intval($this->ident['user_id']);
				$logData['action'] = "Approve Safety Committee MoM";
				$logData['data'] = json_encode($params);
				$logsTable->insertLogs($logData);	
	...
	public function deleteattendancebyidAction() {
		if($this->showAddSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Delete Safety Committee Attendance";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function exporttopdfAction() {	
		...
		$logsTable = $this->loadModel('logs');
		$logData['user_id'] = intval($this->ident['user_id']);
		$logData['action'] = "Export Safety Committee to PDF";
		$logData['data'] = json_encode($params);
		$logsTable->insertLogs($logData);	
		...
		$scComments = $safetyComitteeTable->getCommentsBysafetyComitteeId($params['id'], 0, 'asc');
		
		require_once('fpdf/mc_table.php');
		$pdf=new PDF_MC_Table();
		$pdf->AddPage();
		$pdf->SetTitle($this->ident['initial']." - Safety Committee MoM");
		$pdf->SetFont('Arial','B',15);
		$pdf->Write(10,$this->ident['initial']." - Safety Committee MoM");
		$pdf->ln(10);
			...
			foreach($topic as $to) {
				...
				if(!empty($to['filename']))
				{
					...
					if(!empty($to['images']))
					{
						...
						$topicY = $curY + (ceil($totalLetter/30) * 5);
						$topicX = $curX+16;
				...
				if(!empty($prevScFollowUp[$to['topic_id']]))
				{
					...
					foreach($prevScFollowUp[$to['topic_id']] as $prevfu) {
						if($j < 2)
						{
							...
							$pos = strpos(nl2br($prevfu['follow_up']), '<br');
							if($pos > 0) $fuY = $fuY+5;
							$fuX = $curX+105;
		...
		if(!empty($scComments))
		{
			...
			foreach($scComments as $scmt)
			{
				$sc_comment_date = date("l, j F Y", strtotime(substr($scmt['comment_date'], 0, 10)));
				$pdf->SetFont('Arial','B',8);
				$pdf->SetTextColor(0,0,0);
				$pdf->Cell(10,4,$scmt['name'],0,0,'L');
				$pdf->Ln();
				$pdf->SetFont('Arial','',6);
				$pdf->SetTextColor(170,170,170);
				$pdf->Cell(10,3,$sc_comment_date,0,0,'L');
				$pdf->Ln();
				$pdf->SetFont('Arial','',8);
				$pdf->SetTextColor(0,0,0);
				$pdf->Write(5,str_replace("<br>","\n",$scmt['comment']));
				if(!empty($scmt['filename']))
				{
					$pdf->Ln();
					$pdf->SetTextColor(0,67,187);
					$pdf->write(5,'# '.$scmt['filename']);
					$pdf->Link(10, $pdf->getY(), 189, 5, $this->baseUrl.'/images/hod_meeting/comments_'.substr($scmt['comment_date'], 0, 4).substr($scmt['comment_date'], 5, 2).'/'.$scmt['filename']);
				}
				$pdf->Ln(10);
			}
		}
	...
	public function deletestartdateAction() {
		if($this->allowDeleteSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Delete Safety Committee Topic Start Date";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function deletetargetdateAction() {
		if($this->allowDeleteSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Delete Safety Committee Topic Target Date";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
		...
		if(!empty($sites))
		{
			...
			foreach($sites as $site)
			{
				if($site['site_id'] < 4)
				{
					...
					{
						$html = '<p>Dear GM,</p>
						<p>Please approve Safety Committee MOM below:</p>
						...
						$mail->setSubject($site['initial'] . ' - Safety Committee MOM needs to be approved');
	...
	public function deletetopicimageAction() {
		if($this->allowDeleteSafetyComittee)
		{
			...
			$image = $safetyComitteeTable->getTopicImageById($params['id']);			

			$id = $safetyComitteeTable->deleteTopicImage($params['id']);
			if($id > 0) 
			{
				$datafolder = $this->config->paths->html."/images/safety_comittee";
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Delete Safety Committee Topic Image";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
		}
		else {
	...
	public function addprogressAction() {
		if($this->showAddSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Save Safety Committee Topic Follow Up Progress";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function getfollowupbyidAction() {
		if($this->showAddSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Get Safety Committee Topic Follow By Id";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function deletefollowupimageAction() {
		if($this->allowDeleteSafetyComittee)
		{
			...
			$safetyComitteeTable = $this->loadModel('safetycomittee');
			$image = $safetyComitteeTable->getFollowUpImageById($params['id']);
			
			$params['user_id'] = $this->ident['user_id'];
			$id = $safetyComitteeTable->deleteFollowUpImage($params['id']);
			if($id > 0) 
			{
				$datafolder = $this->config->paths->html."/images/safety_comittee";
				unlink($datafolder.$image['filename']);
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Delete Safety Committee Topic Follow Up Image";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
		}
		else {
	...
	public function addcommentAction() {
		if($this->showSafetyComittee || $this->showSafetyComitteeAdmin)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Add Safety Committee Comment";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function getaccidentreviewbyidAction() {
		if($this->showAddSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Get Safety Committee Topic Accident Review By Id";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function deleteaccidentreviewimageAction() {
		if($this->allowDeleteSafetyComittee)
		{
			...
			$image = $safetyComitteeTable->getAccidentReviewImageById($params['id']);
			
			$id = $safetyComitteeTable->deleteAccidentReviewImage($params['id']);
			if($id > 0) 
			{
				$datafolder = $this->config->paths->html."/images/safety_comittee";
				unlink($datafolder.$image['filename']);
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Delete Safety Committee Topic Accident Review Image";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
		}
		else {
	...
	public function addrecommendationAction() {
		if($this->showAddSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Save Safety Committee Topic Recommendation";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
	...
	public function getrecommendationbyidAction() {
		if($this->showAddSafetyComittee)
		{
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Get Safety Committee Topic Recommendation By Id";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
    ...
    public function deleterecommendationimageAction() {
		if($this->allowDeleteSafetyComittee)
		{
			...
			$image = $safetyComitteeTable->getRecommendationImageById($params['id']);
			
			$id = $safetyComitteeTable->deleteRecommendationImage($params['id']);
			if($id > 0) 
			{
				$datafolder = $this->config->paths->html."/images/safety_comittee";
				unlink($datafolder.$image['filename']);
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Delete Safety Committee Topic Recommendation Image";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	
		}
		else {
	...
	public function exporttopictopdfAction() {	
		...
		$logsTable = $this->loadModel('logs');
		$logData['user_id'] = intval($this->ident['user_id']);
		$logData['action'] = "Export Safety Committee Topic to PDF";
		$logData['data'] = json_encode($params);
		$logsTable->insertLogs($logData);	
		...
		if(!empty($followUp))
		{
			...
			foreach($followUp as $fu) {
				...
				$pdf->SetFont('Arial','',8);
				$pdf->Cell(35,5,'',0,0,'L');
				$pdf->MultiCell(150,5,$fu['follow_up'],0,'L');
				$pdf->Ln();
				$curY = $pdf->getY();
				$fuY = $curY;
				$fuX = $curX+45;
		...
		if(!empty($accidentReview))
		{
			...
			foreach($accidentReview as $ar) {
				...
				$pdf->SetFont('Arial','',8);
				$pdf->Cell(35,5,'',0,0,'L');
				$pdf->MultiCell(150,5,$ar['accident_review'],0,'L');
				$pdf->Ln();
				$curY = $pdf->getY();
		...
		if(!empty($recommendation))
		{
			...
			foreach($recommendation as $rec) {
				...
				$pdf->SetFont('Arial','',8);
				$pdf->Cell(35,5,'',0,0,'L');
				$pdf->MultiCell(150,5,$rec['recommendation'],0,'L');
				$pdf->Ln();
				$curY = $pdf->getY();
