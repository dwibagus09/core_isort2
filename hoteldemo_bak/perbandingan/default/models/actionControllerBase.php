...
class actionControllerBase extends Zend_Controller_Action 
{
	...
	{		
		...
		if(empty($this->ident) && $this->_request->getParam('controller') != "api" && $this->_request->getParam("action") != "login" && $this->_request->getParam("action") != "issueimage" && $this->_request->getParam('action') != "sendreminder" && $this->_request->getParam('action') != "sendweeklyreminderauto" && $this->_request->getParam('action') != "sendweeklyreviewauto" && $this->_request->getParam('action') != "deletegraph"  && $this->_request->getParam('action') != "cacheopenedissuesomments" && $this->_request->getParam('action') != "savesecuritypdf" && $this->_request->getParam('action') != "savesafetyparkingpdf" && $this->_request->getParam('action') != "savehkpdf" && $this->_request->getParam('action') != "saveommodpdf" && $this->_request->getParam('action') != "sendapprovalreminder") {
		...
		if(!empty($id))
		{
			...
			if(!empty($defect_list))
			{
				...
				foreach($defect_list as $dl)
				{
					...
					if (file_exists(str_replace($this->baseUrl."/", $this->config->paths->html, $dl['thumb_pic']))) {
    					list($width, $height) = getimagesize(str_replace($this->baseUrl."/", $this->config->paths->html, $dl['thumb_pic']));
    					if($width > $height)
    					{
    						$w = 18;
    						$h = 0;
			...
			if(!empty($incident))
			{
				...
				foreach($incident as $i)
				{
					...
					if (file_exists(str_replace($this->baseUrl."/", $this->config->paths->html, $i['thumb_pic']))) {
    					list($width, $height) = getimagesize(str_replace($this->baseUrl."/", $this->config->paths->html, $i['thumb_pic']));
    					if($width > $height)
    					{
    						$w = 18;
    						$h = 0;
			...
			if(!empty($glitch))
			{
				...
				foreach($glitch as $g)
				{
					...
					if (file_exists(str_replace($this->baseUrl."/", $this->config->paths->html, $g['thumb_pic']))) {
    					list($width, $height) = getimagesize(str_replace($this->baseUrl."/", $this->config->paths->html, $g['thumb_pic']));
    					if($width > $height)
    					{
    						$w = 18;
    						$h = 0;
			...
			if(!empty($lost_found))
			{
				...
				foreach($lost_found as $lf)
				{
					...
					if (file_exists(str_replace($this->baseUrl."/", $this->config->paths->html, $lf['thumb_pic']))) {
    					list($width, $height) = getimagesize(str_replace($this->baseUrl."/", $this->config->paths->html, $lf['thumb_pic']));
    					if($width > $height)
    					{
    						$w = 18;
    						$h = 0;
