...
class issueClass extends defaultClass
{
	...
	function saveIssue($params) {
		...
		$data = array(
			...
			"floor_tenant_umum" => $params["tenant_public"],
	...
	function saveSolveIssue($params) {
		...
		$data = array(
			...
			"lost_found_option_id" => intval($params["lost_found_option_id"])
		);
	...
	function getIssuesByShift($cat_id, $site_id, $startdate, $enddate) {
		...
		$select = $issuesTable->select()->where('site_id = ?', $site_id)->where('issue_date > ?', $startdate)->where('issue_date <= ?', $enddate);
		$issues = $issuesTable->getAdapter()->fetchAll($select);
		return $issues;
	}
	...
	function getIssuesByModus($month, $year, $category_id) {		
		$issuesTable = new issues(array('db' => 'db'));
		$select = $issuesTable->getAdapter()->select();
	...
	function getTotalIssuesByDayDescending($month, $year, $category_id) {		
		$issuesTable = new issues(array('db' => 'db'));
		$select = $issuesTable->getAdapter()->select();
	...
	function getTotalIssuesByTimePeriode($month, $year, $start_time, $end_time, $category_id) {		
		$issuesTable = new issues(array('db' => 'db'));
		$select = $issuesTable->getAdapter()->select();
	...
	function getTotalIssuesByTenantPublik($month, $year, $tenant_umum, $category_id) {	
		...
		$select->where('i.kejadian_id > ?', 0);
		$select->where('i.modus_id > ?', 0);
		$select->where('i.floor_tenant_umum = ?', $tenant_umum);
		$select->where('i.category_id = ?', $category_id);
	...
	function getPotentialHazardIssues($month, $year) {
		...
		$select->where('floor_tenant_umum  = ?', '1');
		$select->where('i.site_id  = ?', $this->site_id);
	...
	function getTotalIssuesStatBySites($category_id, $year, $months, $days, $period, $floors, $issuetypes, $incidents, $tenant_umum, $modus, $sites) {		
		...
		if($tenant_umum != "") $select->where("i.floor_tenant_umum = ?", $tenant_umum);
	...
	function getMonthlyAnalysisIssues($month, $year, $category_id) {
		$issuesTable = new issues(array('db' => 'db'));
		$select = $issuesTable->getAdapter()->select();
	...
	function getHODIssues($site_ids) {		
		$issuesTable = new issues(array('db' => 'db'));
		$select = $issuesTable->getAdapter()->select();
	...
	function getPotentialHazardFindingsTenant($month, $year) {		
		...
		$select->where("i.floor_tenant_umum = ?", '0');
		$select->order("i.issue_date");
		$issues = $issuesTable->getAdapter()->fetchAll($select);
	...
	function getMonthlyReportIssues($params) { 
		$issuesTable = new issues(array('db' => 'db'));
		$select = $issuesTable->getAdapter()->select();
		$select->from(array("i"=>"issues"), array("i.*"));
		$select->joinLeft(array("c"=>"categories"), "c.category_id = i.category_id", array("c.category_name as department"));
		$select->joinLeft(array("it"=>"issue_type"), "it.issue_type_id = i.issue_type_id", array("it.issue_type"));
		if(!empty($params['category']) && in_array($params['category'], array(1,2,3,5,6,10,11))) 
		{
			if($params['category'] == 1) 
		...
		$select->where('month(i.issue_date) = ?', $params['month']);
		$select->where('year(i.issue_date) = ?', $params['year']);
		$select->where('i.category_id = ?', $params['category']);
		$select->order("issue_date asc");
		$issues = $issuesTable->getAdapter()->fetchAll($select);
		return $issues;
	}
