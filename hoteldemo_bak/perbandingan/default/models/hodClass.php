...
class hodClass extends defaultClass
{
	...
	function getHodMeetingTopics($hod_meeting_id, $hod_meeting_date = "", $site_ids = 0)
	{
		$hodMeetingTopicTable = new hod_meeting_topic(array('db'=>'db'));
		...
		if(!empty($site_ids)) $select->where("si.site_id IN (".$site_ids.")");
	...
	function getHodMeetingTopicById($topic_id)
	{
		...
		$select = $hodMeetingTopicTable->getAdapter()->select();
		$select->from(array("h"=>"hod_meeting_topic"), array("h.*"));
		$select->joinLeft(array("s"=>"sites"), "s.site_id = h.site_id", array("s.site_id", "s.site_name"));
	...
	function getAttendanceByMeetingSiteId($hod_meeting_id, $site_id)
	{
		$hodMeetingAttendanceTable = new hod_meeting_attendance(array('db'=>'db'));
		$select = $hodMeetingAttendanceTable->select()->where('hod_meeting_id = ?', $hod_meeting_id)->where('site_id = ?', $site_id);
		$attendance = $hodMeetingAttendanceTable->getAdapter()->fetchAll($select);
		return $attendance;
	}	
}
