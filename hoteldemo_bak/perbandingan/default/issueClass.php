...
class issueClass extends defaultClass
{
	...
	function getTotalIssuesStatBySites($category_id, $year, $months, $days, $period, $floors, $incidents, $tenant_umum, $modus, $sites) {		
		$issuesTable = new issues(array('db' => 'db'));
		$select = $issuesTable->getAdapter()->select();
	...
	function getTotalIssuesStatByTimePeriode($start_time, $end_time, $category_id, $year, $months, $days, $period, $floors, $incidents, $tenant_umum, $modus, $sites) {		
		$issuesTable = new issues(array('db' => 'db'));
		$select = $issuesTable->getAdapter()->select();
		$select->from(array("i"=>"issues"), array("count(issue_id) as total"));
		if($category_id == 1) 
	...
	function getPotentialHazardFindingsTenant($month) {		
		$issuesTable = new issues(array('db' => 'db'));
		$select = $issuesTable->getAdapter()->select();
