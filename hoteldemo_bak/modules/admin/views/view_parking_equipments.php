	...
	<div class="outer">
		<div class="inner bg-light lter">
			<div class="col-lg-12">
				...
				<h3 class="page-title">Parking &amp; Traffic Equipments</h3>
					...
					<table class="table table-striped responsive-table">
						...
						?>
							<tr>
								...
								<td><a href="#equipment-form" class="edit-equipment action-btn" data-id="<?php echo $equipment['parking_equipment_list_id']; ?>"><i class="fa fa-edit" ></i></a>
									<a class="action-btn delete-equipment" data-id="<?php echo $equipment['parking_equipment_list_id']; ?>"><i class="fa fa-trash" ></i></a>
								</td>
							</tr>
						<?php $i++; } ?>
						</tbody>  
...
<!-- comment form -->
  <form action="" id="equipment-form" class="mfp-hide white-popup-block" >
	<div id="err-msg"></div>
	<input type="hidden" name="parking_equipment_list_id" id="parking_equipment_list_id" />
...
$(document).ready(function() {
	...
	$('.edit-equipment').magnificPopup({
		...
		callbacks: {
			open: function() {
				var id = selectedID;
				$.ajax({
					url: "/admin/equipment/getparkingequipmentbyid",
					data: { id : id }
				}).done(function(response) { 
					var resp = jQuery.parseJSON(response);
					$("#parking_equipment_list_id").val(resp.data.parking_equipment_list_id);
	...
	$('#equipment-form').on('submit', function(event){
		...
		$.ajax({
			url: '/admin/equipment/addparkingequipment',
			type: 'POST',
			data: $(this).serialize(),
			success: function(id) {
				 location.href="/admin/equipment/viewparkingequipment<?php echo $this->type; ?>";
			}
		});
	});
	...
	$('.delete-equipment').click(function() {
		...
		{
			$.ajax({
				url: "/admin/equipment/deleteparkingequipment",
