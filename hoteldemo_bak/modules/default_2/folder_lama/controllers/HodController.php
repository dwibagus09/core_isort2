...
class HodController extends actionControllerBase
{
	...
	public function addAction() {
		if($this->showAddHOD)
		{
			...
			if($this->site_id == 2) $categories = $category->getSelectedCategories("1, 2, 5, 6, 11, 23, 28, 29, 30, 31, 32, 33, 34, 35");
			else $categories = $category->getSelectedCategories("2, 6, 23, 28, 29, 30, 31, 32, 33, 34");
			if(!empty($categories))
			{
				foreach($categories as &$cat)
				{
					if($this->site_id == 1)
					{
						if($cat['category_id'] == 2) $cat['category_name'] = "Housekeeping SPV";
						if($cat['category_id'] == 6) $cat['category_name'] = "Engineering Operational SPV";
					}
					else
					{
						if($cat['category_id'] == 1) $cat['category_name'] = "HOD Security & Safety";
						if($cat['category_id'] == 2) $cat['category_name'] = "HOD Housekeeping";
						if($cat['category_id'] == 5) $cat['category_name'] = "HOD Fit Out";
						if($cat['category_id'] == 6) $cat['category_name'] = "HOD Engineering";
						if($cat['category_id'] == 11) $cat['category_name'] = "HOD Tenant Relation";
					}
				}
			}
			$this->view->categories = $categories;

            $this->view->title = "Add Coordination Meeting MOM";
	...
	public function editAction() {
		if($this->showAddHOD)
		{
            ...
            $this->view->title = "Edit Coordination Meeting MOM";
	...
	public function saveAction() {
		if($this->showAddHOD)
		{
			...
			$i = 0;
			foreach($params['attendance_cat_id'] as $category_id)
			{
	...
	public function hodmeetingform2Action() {
		if($this->showAddHOD)
		{
			...
			foreach($topic as &$t)
			{
				...
				if($t['category_id'] == 2) $t['category_name'] = "Housekeeping SPV";
				if($t['category_id'] == 6) $t['category_name'] = "Engineering Operational SPV";
			}
	...
	public function viewdetailAction() {
		if($this->showHODMeeting == 1 || $this->showHODMeetingAdmin == 1)
		{
			...
			if(!empty($attendance))
			{
				foreach($attendance as $att)
				{
					$pic[$att['category_id']] .= "<br/>".$att['attendance_name'];
				}
			}
			...
			if(!empty($cat_ids))
			{
				...
				{
					if($ci['category_id'] > 0 && $this->ident['role_id'] != 5)
						$dept_ids .= $ci['category_id'].",";
					else
					{
						$userCatId = $ci['category_id'];
						$dept_ids = "";
						break;
					}
				}
	...
	public function exporttopdfAction() {	
		...
		$pdf->AddPage();
		if($this->teacher) $pdf->SetTitle($this->ident['initial']." - Coordination Meeting MOM");
		else $pdf->SetTitle($this->ident['initial']." - Coordination Meeting MOM");
		$pdf->SetFont('Arial','B',15);
		if($this->teacher) $pdf->Write(10,$this->ident['initial']." - Coordination Meeting MOM");
		else  $pdf->Write(10,$this->ident['initial']." - Coordination Meeting MOM");
		$pdf->ln(10);
		...
		if(!empty($issues))
		{
			...
			$pdf->Cell(20,7,'Department',1,0,'C',true);
			...
			foreach($issues as $issue) { 	
				...
				$x1 = $pdf->GetY();
				$pdf->Row(array($issue['category_name'].$pic[$issue['category_id']],$issue['description'],"\n\n\n\n",$issue['location'],$issue['date'], $comments));
				$x2= $pdf->GetY();
				if($x2<$x1) $y = 11;
				...
				if(!empty($issue['picture']) && @getimagesize($issueImagePath.str_replace(".", "_thumb.", $issue['picture']))) {
					...
					if(in_array($pic[count($pic)-1], array("jpg", "jpeg")))
					{
						...
						if($exifimgkaizen['Orientation'] == '3'  || $exifimgkaizen['Orientation'] == '6' || $exifimgkaizen['Orientation']=='8')
						{
							...
							$pdf->Image($issueImagePath.str_replace(".", "_thumb.", $issue['picture']),76,$y-16, $w,$h);
							$pdf->Rotate(0);
						}
						else
		...
		if(!empty($topic))
		{
			$pdf->SetFont('Arial','B',9);
			$pdf->SetTextColor(0,0,0);
			$pdf->Write(10,"Project List");
			$pdf->Ln();
			$pdf->SetFont('Arial','B',8);
			$pdf->SetFillColor(158,130,75);
			$pdf->SetTextColor(255,255,255);
			$pdf->Cell(20,7,'Department',1,0,'C',true);
			$pdf->Cell(60,7,'Project Name',1,0,'C',true);
			...
			foreach($topic as $to) {
				...
				$pdf->Row(array($to['category_name'].$pic[$to['category_id']],$to['topic'],$to['targetdate'],$to['startdate'], $to['finishdate'], $prevFollowUpTopic));	
			}
		}
		...
		if(!empty($sites))
		{
			...
			foreach($sites as $site)
			{
				if($site['site_id'] < 4)
				{
					...
					{
						$html = '<p>Dear GM,</p>
						<p>Please approve Coordination Meeting MOM below:</p>
						...
						$mail->setSubject($site['initial'] . ' - Coordination Meeting MOM needs to be approved');
	...
	public function exporttopictopdfAction() {	
		...
		$pic = $hodTable->getAttendanceByMeetingSiteId($topic['hod_meeting_id'], $topic['department_id']);
		...
		if($topic['category_id'] == 2) $topic['category_name'] = "Housekeeping SPV";
		if($topic['category_id'] == 6) $topic['category_name'] = "Engineering Operational SPV";
		...
		$pdf=new PDF_MC_Table();
		$pdf->AddPage();
		$pdf->SetTitle("Coordination Meeting MOM Project/Issue");
		$pdf->SetFont('Arial','B',15);
		$pdf->Write(10,"Coordination Meeting MOM Project/Issue");
		$pdf->ln(10);
