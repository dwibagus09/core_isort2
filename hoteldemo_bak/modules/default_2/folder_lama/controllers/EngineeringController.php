...
class EngineeringController extends actionControllerBase
{
	...
	public function addmonthlyanalysisAction() {
		if($this->addEngineeringMonthlyAnalysis)
		{
			...
			if(!empty($params['id'])) 
			{
				...
				$monthly_analysis = $engineeringClass->geMonthlyAnalysisById($params['id']);
			...
			//$modus = $this->cache->load("modus_".$this->site_id."_6_".$ym);
			if(empty($modus))
			{		
				Zend_Loader::LoadClass('modusClass', $this->modelDir);
				$modusClass = new modusClass();	
				$modus = $modusClass->getModus('6');
				//$this->cache->save($modus, "modus_".$this->site_id."_2_".$ym, array("modus_".$this->site_id."_6_".$ym), 0);
			}
			
			//$totalModusPerMonth = $this->cache->load("total_modus_per_month_".$this->site_id."_6_".$ym);
			if(empty($totalModusPerMonth))
			{	
				$totalModusPerMonth =  array();
				for($b=1; $b<=$m; $b++) // get rekapitulasi utk bulan ini dan bulan2 sblmnya
				{
					$totalModus = $issueClass->getIssuesByModus($b, $y, '6');
				...
				$totalModus = $issueClass->getIssuesByModus($m, $y, '6');
				foreach($totalModus as $tm) {
			...
			foreach($modus as $mo)
			{
				if($mo['kejadian'] != $modus[$k-1]['kejadian'])
				{
					...
					$analisa_hari = $issueClass->getMonthlyIssuesByDay($mo['kejadian_id'], $m, $y, '6');
					...
					$rekap[$i]['analisa_jam'][0] = $issueClass->getMonthlyIssuesByTime($mo['kejadian_id'], $m, $y, '09:00:00', '12:00:00', '6');
					$rekap[$i]['analisa_jam'][1] = $issueClass->getMonthlyIssuesByTime($mo['kejadian_id'], $m, $y, '12:00:00', '16:00:00', '6');
					$rekap[$i]['analisa_jam'][2] = $issueClass->getMonthlyIssuesByTime($mo['kejadian_id'], $m, $y, '16:00:00', '19:00:00', '6');
					$rekap[$i]['analisa_jam'][3] = $issueClass->getMonthlyIssuesByTime($mo['kejadian_id'], $m, $y, '19:00:00', '23:00:00', '6');
					$rekap[$i]['analisa_jam'][4] = $issueClass->getMonthlyIssuesByTime($mo['kejadian_id'], $m, $y, '23:00:00', '09:00:00', '6');
					$j = 0;
					$i++;	
				}
				...
				$uraian_kejadian = $issueClass->getIssuesByModusId($mo['modus_id'], $m, $y, '6');
			...
			$this->view->urutan_hari_tertinggi = $issueClass->getTotalIssuesByDayDescending($m, $y, '6');
			$urutan_total_jam[0] = $issueClass->getTotalIssuesByTimePeriode($m, $y, '09:00:00', '12:00:00', '6');
			$urutan_total_jam[1] = $issueClass->getTotalIssuesByTimePeriode($m, $y, '12:00:00', '16:00:00', '6');
			$urutan_total_jam[2] = $issueClass->getTotalIssuesByTimePeriode($m, $y, '16:00:00', '19:00:00', '6');
			$urutan_total_jam[3] = $issueClass->getTotalIssuesByTimePeriode($m, $y, '19:00:00', '23:00:00', '6');
			$urutan_total_jam[4] = $issueClass->getTotalIssuesByTimePeriode($m, $y, '23:00:00', '09:00:00', '6');
			arsort($urutan_total_jam);

			$this->view->urutan_total_jam = $urutan_total_jam;
			
			$this->view->incidents = $issueClass->getEngineeringIssueSummary($m, $y, $params['id']);
			/*$urutan_total_issue_tenant = $issueClass->getTotalIssuesByTenantPublik($m, $y, '0', '6');
			if(!empty($urutan_total_issue_tenant))
			{
				...
				{
					$data = array();
					$data = $issueClass->getTotalIssuesByLocation($m, $y, '0', $t['location'], $t['floor_id'], '6');
					if(!empty($data))
					{
						foreach($data as $dt)
						{
			...
			$urutan_total_issue_publik = $issueClass->getTotalIssuesByTenantPublik($m, $y, '1', '6');
			if(!empty($urutan_total_issue_publik))
			{
				...
				{
					$data = array();
					$data = $issueClass->getTotalIssuesByLocation($m, $y, '1', $p['location'], $p['floor_id'], '6');
			...
			$list_tangkapan = $issueClass->getPelakuTertangkapByCategory($y, '6');
			...
			$pelaku_tertangkap_detail = $issueClass->getPelakuTertangkapDetail($m, $y, '6');
			...
			$listIssues = $issueClass->getMonthlyAnalysisIssues($m, $y, '6');
			...
			$this->view->listKejadian = $incidentClass->getIncidentByCategoryId('6');


			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Add Engineering Monthly Analysis";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	

			$this->renderTemplate('form_engineering_monthly_analysis.tpl'); 
		}
		else
		{
	...
	public function savemonthlyanalysisAction() {
		...
		if(empty($params['monthly_analysis_id']))
		{
			Zend_Loader::LoadClass('engineeringClass', $this->modelDir);
			$engineeringClass = new engineeringClass();
			$params['monthly_analysis_id'] = $engineeringClass->saveMonthlyAnalysis($params);
		}
		
		Zend_Loader::LoadClass('issueClass', $this->modelDir);
		$issueClass = new issueClass();
		$this->view->incidents = $issueClass->getEngineeringIssueSummary(date("m"), date("Y"));
		...
		foreach($params['summary_id'] as $summary_id)
		{
			...
			$monthlyanalysissummaryClass->saveMonthlyAnalysis($data, '6');
			$i++;
		}

		$logsTable = $this->loadModel('logs');
		$logData['user_id'] = intval($this->ident['user_id']);
		$logData['action'] = "Save Engineering Monthly Analysis";
		$logData['data'] = json_encode($params);
		$logsTable->insertLogs($logData);	

		$this->_response->setRedirect($this->baseUrl.'/default/engineering/viewmonthlyanalysis');
		$this->_response->sendResponse();
		exit();
	}
	...
	public function viewmonthlyanalysisAction() {
		...
		Zend_Loader::LoadClass('engineeringClass', $this->modelDir);
		$engineeringClass = new engineeringClass();
		$monthlyAnalysis = $engineeringClass->getMonthlyAnalysis($params);
		...
		$totalMonthlyAnalysis = $engineeringClass->getTotalMonthlyAnalysis();
		if($totalMonthlyAnalysis > 10)
		{
			if($params['start'] >= 10)
			{
				$this->view->firstPageUrl = "/default/engineering/viewmonthlyanalysis";
				$this->view->prevUrl = "/default/engineering/viewmonthlyanalysis/start/".($params['start']-$params['pagesize']);
			}
			if($params['start'] < (floor(($totalMonthlyAnalysis-1)/10)*10))
			{
				$this->view->nextUrl = "/default/engineering/viewmonthlyanalysis/start/".($params['start']+$params['pagesize']);
				$this->view->lastPageUrl = "/default/engineering/viewmonthlyanalysis/start/".(floor(($totalMonthlyAnalysis-1)/10)*10);
			}
		}
		...
		$logsTable = $this->loadModel('logs');
		$logData['user_id'] = intval($this->ident['user_id']);
		$logData['action'] = "View Engineering Monthly Analysis List";
		$logData['data'] = "";
		$logsTable->insertLogs($logData);	

		$this->renderTemplate('view_engineering_monthly_analysis.tpl'); 
	}
	...
	public function viewdetailmonthlyanalysisAction() {
		...
		if(!empty($params['id'])) 
		{
			...
			//$modus = $this->cache->load("modus_".$this->site_id."_6_");
			if(empty($modus))
			{		
				Zend_Loader::LoadClass('modusClass', $this->modelDir);
				$modusClass = new modusClass();	
			...
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "View Engineering Monthly Analysis Detail";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);	

			//echo "<pre>"; print_r($pelaku_tertangkap_detail); exit();
			$this->renderTemplate('engineering_monthly_analysis_detail.tpl'); 
		}
	}

	public function downloadengineeringmonthlyanalysisAction() {		
		$params = $this->_getAllParams();
		...
		{			
			$logsTable = $this->loadModel('logs');
			$logData['user_id'] = intval($this->ident['user_id']);
			$logData['action'] = "Export Engineering Monthly Analysis Report to PDF";
			$logData['data'] = json_encode($params);
			$logsTable->insertLogs($logData);
			...
			if(!empty($modus))
			{
				...
				$urutan_hari_tertinggi = $issueClass->getTotalIssuesByDayDescending($m, $y, '6');
				$urutan_total_jam[0] = $issueClass->getTotalIssuesByTimePeriode($m, $y, '09:00:00', '12:00:00', '6');
				$urutan_total_jam[1] = $issueClass->getTotalIssuesByTimePeriode($m, $y, '12:00:00', '16:00:00', '6');
				$urutan_total_jam[2] = $issueClass->getTotalIssuesByTimePeriode($m, $y, '16:00:00', '19:00:00', '6');
				$urutan_total_jam[3] = $issueClass->getTotalIssuesByTimePeriode($m, $y, '19:00:00', '23:00:00', '6');
				...
				$incidents = $issueClass->getEngineeringIssueSummary($m, $y, $params['id']);
				...
				if(!empty($urutan_total_issue_tenant))
				{
					...
					{
						$data = array();
						$data = $issueClass->getTotalIssuesByLocation($m, $y, '0', $tenant['location'], $tenant['floor_id'], '6');
						if(!empty($data))
						{
							foreach($data as $dt)
							{
			...
			$pdf->SetTitle($this->ident['initial']." - ENGINEERING MONTHLY ANALYTICS - ".$monthYear);
			$pdf->SetFont('Arial','B',12);
			$pdf->Write(5, $this->ident['initial']." - ENGINEERING MONTHLY ANALYTICS - ".$monthYear);
			$pdf->ln();
						...
						foreach($rekapitulasi['modus'] as $mo) {
							if(!empty($mo['uraian_kejadian']))
							{
								...
								foreach($mo['uraian_kejadian'] as $uk) {		
									...
									else if($k > count($mo['uraian_kejadian'])) 
									{
										$k = 0;
