	...
	<div class="row">
		<div class="col-md-12 col-sm-12 col-xs-12">
			<div style="margin-bottom:10px;">
				<h2 class="pagetitle"><?php if($this->category_id==6) echo $this->ident['initial']." - Preventive Maintenance "; else echo $this->ident['initial']." - ".$this->category['category_name']." Action Plan "; ?></h2>
	...
	<div id="view-all" class="mfp-hide white-popup-block">
		<div style="float:right; margin-top: 7px;">
			<select id="year_list" name="year_list">
				...
				<?php for($i=$this->start_year; $i<=date("Y"); $i++) { ?>
  ...
  <form action="" id="popup-form" class="mfp-hide white-popup-block" autocomplete="off">
		<div id="show_form">
			...
			<select id="year" name="year">
				<option value="">- Select Year -</option>
				<?php for($y = $this->start_year; $y <= date("Y")+1; $y++) {?>
...
var calendar = new FullCalendar.Calendar(calendarEl, {
  ...
  ?>
	{
	  title: '<?php echo addslashes($schedule['activity_name']).' (' . round($schedule['percentage'],2).'%) ('.intval($schedule['totalDone']).'/'.intval($schedule['total']).')'; ?>',
  ...
  eventClick: function(info) {
	...
	$.ajax({
		...
		var schedule = jQuery.parseJSON(response);			
		$( "#schedule-form-title" ).html(schedule.target_name+" - "+schedule.activity_name+" ("+schedule.percentage+"%) ("+schedule.totalDone+"/"+schedule.total+")<br/>"+schedule.date );
		var superAdmin = 1;
...
$(".fc-prev-button").click(function(event){
	...
	var curMonth = String(y) + String(m);
	if(alreadyLoad[curMonth] != 1)
	{
		$("body").mLoading();
		$.ajax({
