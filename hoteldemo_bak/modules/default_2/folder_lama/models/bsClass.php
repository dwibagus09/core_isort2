...
{	
	function saveMonthlyAnalysis($params) {
		$monthlyAnalysisTable = new building_service_monthly_analysis(array('db'=>'db'));
	...
	function getMonthlyAnalysis() {
		...
		$select->from(array("m"=>"building_service_monthly_analysis"), array("m.*"));
	...
	function getTotalMonthlyAnalysis() {
		...
		$select = "select count(*) as total from building_service_monthly_analysis where site_id =".$this->site_id;
		$monthlyAnalysis = $monthlyAnalysisTable->getAdapter()->fetchRow($select);
		return $monthlyAnalysis['total'];
	}

	function getBuildingServiceMonthlyAnalysisByMonthYear($m, $y) {
		$monthlyAnalysisTable = new building_service_monthly_analysis(array('db'=>'db'));
