...
class trClass extends defaultClass
{	
	function saveMonthlyAnalysis($params) {
		$monthlyAnalysisTable = new tenant_relation_monthly_analysis(array('db'=>'db'));
	...
	function getMonthlyAnalysis() {
		...
		$select->from(array("m"=>"tenant_relation_monthly_analysis"), array("m.*"));
	...
	function getTotalMonthlyAnalysis() {
		...
		$select = "select count(*) as total from tenant_relation_monthly_analysis where site_id =".$this->site_id;
		$monthlyAnalysis = $monthlyAnalysisTable->getAdapter()->fetchRow($select);
		return $monthlyAnalysis['total'];
	}

	function getTenantRelationMonthlyAnalysisByMonthYear($m, $y) {
		$monthlyAnalysisTable = new tenant_relation_monthly_analysis(array('db'=>'db'));
