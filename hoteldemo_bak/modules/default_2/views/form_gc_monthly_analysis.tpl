...
<?php 
if(empty($this->gc['report_date'])) $cur_date = date("Y-m-d");
else {
	$cur_report_date = explode(" ",$this->gc['report_date']);
  ...
  <div class="detail-report">
	...
	<div class="row">
	  <div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel">
		  <div class="x_title">
			<h2 class="pagetitle">Guest Complain Monthly Analytics <?php echo $this->monthYear; ?></h2>
		  ...
		  <div class="x_content">
			...
			<table id="perlengkapan-table" class="table">
			  ...
			  <tbody>
				...
				<?php if($j == 0 || $j > $rekapitulasi['total_modus'])
					{	$j = 0;
				?>
					<td rowspan="<?php echo $rekapitulasi['total_modus']; ?>"><?php echo $rekapitulasi['category']; ?></td>
				<?php } ?>
			...
			<form class="form-horizontal form-label-left" action="/default/gc/savemonthlyanalysis" method="POST" onSubmit="$('body').mLoading();">
			  ...
			  <span class="section">KESIMPULAN UMUM</span>
				...
				<tbody>
					...
					<tr>
						...
						<td><?php echo $incident['category']; ?><input type="hidden" name="category_id[]" value="<?php echo $incident['category_id']; ?>"></td>
						<td><?php echo $incident['modus']; ?><input type="hidden" name="modus_id[]" value="<?php echo $incident['modus_id']; ?>"></td>
						<td align="center"><?php echo $incident['total_modus']; ?></td>
			...
			open: function() {
			  $.ajax({
					...
					$.ajax({
						...
						data: { kejadian_id : resp.kejadian_id, category_id : '11' }
					}).done(function(response) {
						$("#modus_id").empty();
					...
					$("#kejadian_id").change(function() {
						...
						{			
							...
							$.ajax({
								url: "/default/issue/getmodusbykejadianid",
								data: { kejadian_id : $( this ).val(), category_id: '11'  }
							}).done(function(response) {
								$("#modus_id").empty();
								var object = $.parseJSON(response);
								$.each(object, function (item, value) {
	...
	$('#issue-form').on('submit', function(event){
		...
		$.ajax({
			...
			success: function() {
				...
				<?php } ?>
				location.href="/default/tr/addmonthlyanalysis"+idparam+"?"+Math.random()+"#issue"+issue_id;
			}
		});
	});
