						...
						<table id="attendance-table" class="table table-striped">
							...
							<?php if(!empty($this->attendance)) {
								...
								{ ?>
									<tr>
										<td>
										    <select name="attendance_cat_id[]" class="form-control col-md-7 col-xs-12" required>
										        <option value="" disabled selected hidden>Select Department</option>
                        					    <?php foreach($this->categories as $category) { ?>
                        						    <option value="<?php echo $category['category_id']; ?>" <?php if($attendance['category_id'] == $category['category_id']) echo 'selected="selected"'; ?> ><?php echo $category['category_name']; ?></option>
                        						<?php } ?>
                        					</select>
										</td>
...
$(document).ready(function() {
	...
	$("#add-attendance").click(function() {
		...
		row = '<tr>
				<td>
				    <select id="category-select" name="attendance_cat_id[]" class="form-control col-md-7 col-xs-12" required>
				        <option value="" disabled selected hidden>Select Department</option>
					    <?php foreach($this->categories as $category) { ?>
