...
class userClass extends defaultClass
{
	...
	function getTotalUserPerDepartment($cat_id, $site_id = 0) {
		...
		switch($cat_id)
		{
			...
			case 1: $select->where("u.role_id like '2,%' or u.role_id = '2' or u.role_id like '%,2' or u.role_id like '%,2,%' or u.role_id like '3,%' or u.role_id = '3' or u.role_id like '%,3' or u.role_id like '%,3,%'");
					break;
			// Housekeeping
			case 2: $select->where("u.role_id like '9,%' or u.role_id = '9' or u.role_id like '%,9' or u.role_id like '%,9,%' or u.role_id like '15,%' or u.role_id = '15' or u.role_id like '%,15' or u.role_id like '%,15,%'");
					break;

			// Safety
			case 3: $select->where("u.role_id like '7,%' or u.role_id = '7' or u.role_id like '%,7' or u.role_id like '%,7,%' or u.role_id like '14,%' or u.role_id = '14' or u.role_id like '%,14' or u.role_id like '%,14,%'");
					break;

			// Parking & Traffic
			case 5: $select->where("u.role_id like '8,%' or u.role_id = '8' or u.role_id like '%,8' or u.role_id like '%,8,%' or u.role_id like '16,%' or u.role_id = '16' or u.role_id like '%,16' or u.role_id like '%,16,%'");
					break;
					
			// Engineering
			case 6: $select->where("u.role_id like '21,%' or u.role_id = '21' or u.role_id like '%,21' or u.role_id like '%,21,%' or u.role_id like '31,%' or u.role_id = '31' or u.role_id like '%,31' or u.role_id like '%,31,%'");
					break;
		}
	...
	function getDepartmentsByRoles($roles) {
		$roleTable = new role(array('db'=>'db'));
		$select = $roleTable->select();
		$select->where('role_id IN ('.$roles.')');	
		$dept = $roleTable->getAdapter()->fetchAll($select);
		return $dept;
	}
	
	function getAllUsersName($site_id) {
		$userTable = new users(array('db'=>'db'));
		$select = $userTable->getAdapter()->select();
		$select->from(array("u"=>"users"), array("u.user_id", "u.name"));
		$select->where('site_id = ?', $site_id);			
		$users = $userTable->getAdapter()->fetchAll($select);
		return $users;
	}
	
	function getAllUsersNameDept($site_id) {
		$userTable = new users(array('db'=>'db'));
		$select = $userTable->getAdapter()->select();
		$select->from(array("u"=>"users"), array("u.user_id", "u.name"));
		$select->joinLeft(array("r"=>"role"), "r.role_id = u.role_id", array());
		$select->joinLeft(array("c"=>"categories"), "c.category_id = r.category_id", array("c.category_name"));
		$select->where('u.site_id = ?', $site_id);			
		$users = $userTable->getAdapter()->fetchAll($select);
		return $users;
	}
}