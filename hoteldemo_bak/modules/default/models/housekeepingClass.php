...
{	
	...
	function saveMonthlyAnalysis($params) {
		$monthlyAnalysisTable = new housekeeping_monthly_analysis(array('db'=>'db'));
	...
	function getMonthlyAnalysis() {
		...
		$select->from(array("m"=>"housekeeping_monthly_analysis"), array("m.*"));
	...
	function getTotalMonthlyAnalysis() {
		...
		$select = "select count(*) as total from housekeeping_monthly_analysis where site_id =".$this->site_id;
		$monthlyAnalysis = $monthlyAnalysisTable->getAdapter()->fetchRow($select);
		return $monthlyAnalysis['total'];
	}

	function getHousekeepingMonthlyAnalysisByMonthYear($m, $y) {
		$monthlyAnalysisTable = new housekeeping_monthly_analysis(array('db'=>'db'));
