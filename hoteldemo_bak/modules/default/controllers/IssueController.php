...
&#128279; <a href="'.$this->config->general->url."default/issue/listissues/s/".$params['site_id']."/c/".$params['category']."/id/".$id.'">Open Kaizen</a>',
				...
				// SEND Notification to Front Office Telegram Channel for GRC
				$foCategory = $categoryTable->getCategoryById(16);	
				$foChatId=$foCategory['site_'.$params['site_id']];
				
				$paramsTelegramFo=array(
					'chat_id'=>$foChatId,
					'text'=> '&#128227; <b><u>NEW KAIZEN</u></b> &#128227;
&#128483; '.$this->ident['name'].'
&#128205; '.$area.$floor.' 
...
&#128279; <a href="'.$this->config->general->url."default/issue/listissues/s/".$params['site_id']."/c/".$params['category']."/id/".$id.'">Open Kaizen</a>',
				...
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
				curl_setopt($ch, CURLOPT_POST, 1);
				curl_setopt($ch, CURLOPT_POSTFIELDS, ($paramsTelegramFo));
				curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
				curl_setopt($ch,CURLOPT_CONNECTTIMEOUT, 3);
				curl_setopt($ch,CURLOPT_TIMEOUT, 30);
				$result = curl_exec($ch);
			...
			else
			{
				if (move_uploaded_file($file["tmp_name"],$datafolder."/".$fileName)){
					...
					$new_file_thumb = $curDate."_thumb.jpg";
					$new_file_large = $curDate."_large.jpg";
	...
	function listissuesAction()
	{
		...
		if(empty($params['category']) && $params['category']!='0')
		{
				if(count($this->ident['role_ids']) == 1)
				{
					...
					if($userRole['category_id'] > 0 && (($this->site_id == 1 && in_array($userRole['category_id'], array(2, 6, 10))) || ($this->site_id == 2 && in_array($userRole['category_id'], array(1, 2, 5, 6, 10, 11)))))	$params['category'] = $userRole['category_id'];
				} 
		}
		...
		else if(empty($params['category'])) $this->view->selectedCategory = 2;
		...
		//$params['category'] = 6; // Engineeringwo
		//$this->view->totalEngWoIssues = $issueClass->getTotalPendingIssues('0', $this->site_id, $params);
	...
	function submitsolveissueAction()
	{
		...
		if ($file["error"] > 0) {
			...
			if (move_uploaded_file($file["tmp_name"],$datafolder."/".$fileName)){
				...
				$checklistClass->updateIssueId($params['issue_id'], 0);
...
&#128279; <a href="'.$this->config->general->url."default/issue/solvedissues/s/".$this->site_id."/c/".$issue['category_id']."/id/".$params['issue_id'].'">Open Kaizen</a>';
				...
				if($type['issue_type_id'] == 19) // SEND Notification to Front Office Telegram Channel for GRC
				{
					$foCategory = $categoryTable->getCategoryById(16);	
					$foChatId=$foCategory['site_'.$this->site_id];
				...
				$this->getResponse()->setRedirect($this->config->general->url."default/issue/solvedissues/id/".$id);
				$this->getResponse()->sendResponse();
				exit;
			}	
			else {
				...
				$this->getResponse()->setRedirect($this->config->general->url."default/issue/listissues/err/1");
				$this->getResponse()->sendResponse();
				exit;			
			}
		}
...
&#128279; <a href="'.$this->config->general->url."default/issue/listissues/s/".$selIssue['site_id']."/c/".$selIssue['category_id']."/id/".$selIssue['issue_id'].'">Open Kaizen</a>',
				...
				{
					$foCategory = $categoryTable->getCategoryById(16);	
					$foChatId=$foCategory['site_'.$selIssue['site_id']];
	...
	&#128279; <a href="'.$this->config->general->url."default/issue/listissues/s/".$selIssue['site_id']."/c/".$selIssue['category_id']."/id/".$selIssue['issue_id'].'">Open Kaizen</a>',
				...
				$this->getResponse()->setRedirect($this->config->general->url."default/issue/listissues/s/".$selIssue['site_id']."/c/".$selIssue['category_id']."/id/".$params['progress_issue_id']);
				$this->getResponse()->sendResponse();
				exit;
			}	
			else {
		...
		if(!empty($comments)) {
			foreach($comments as $comment)
			{
				...
				if($comment['user_id'] != ucwords(strtolower($this->ident['user_id']))){
					$commentText .= '<div class="msg-chat rcvd" ><strong>'.$comment['name'].' : </strong>'.$comment['comment'].'<br/>'.$comment['comment_date'];
					if(!empty($comment['filename'])) $commentText .='<a href="'.$this->baseUrl."/comments/".substr($comment['comment_date'],0,4)."/".$comment['filename'].'" target="_blank"><i class="fa fa-paperclip"></i> '.$comment['filename'].'</a>';
					$commentText .= '<div class >'.$comment['comment_date_']."</div></div>";	
				}elseif($comment['user_id'] == ucwords(strtolower($this->ident['user_id']))){
					$commentText .= '<div class="msg-chat sent" ><strong>'.$comment['name'].' : </strong>'.$comment['comment'].'<br/>'.$comment['comment_date'];
					if(!empty($comment['filename'])) $commentText .='<a href="'.$this->baseUrl."/comments/".substr($comment['comment_date'],0,4)."/".$comment['filename'].'" target="_blank"><i class="fa fa-paperclip"></i> '.$comment['filename'].'</a>';
					$commentText .= '<div class >'.$comment['comment_date_']."</div></div>";	
				}
...
&#128279; <a href="'.$this->config->general->url."default/issue/".$page."/s/".$this->site_id."/c/".$issue['category_id']."/id/".$issue['issue_id'].'">Open Kaizen</a>';
		...
		if($type['issue_type_id'] == 19) // SEND Notification to Front Office Telegram Channel for GRC
		{
			...
			$foCategory = $categoryClass->getCategoryById(16);	
			$foChatId=$foCategory['site_'.$params['site_id']];
...
*/	}	
	...
	function getissuetypeandfloorbycatidAction()
	{
		...
		if($this->site_id == 1)
		{
			if(in_array($this->ident['role_id'], array(32,33)))
				$inc = "19";
			else
				$inc = "14,15,16";
		}
		...
		Zend_Loader::LoadClass('issuetypeClass', $this->modelDir);
		$issuetypeClass = new issuetypeClass();
		$issue_type = $issuetypeClass->getIssueTypeByCategoryId($params['category_id'], $inc);
			...
			foreach($issues as $issue) {
				...
				if(!empty($comments)) { 
					...
					foreach($comments as $comment)
					{
						...
						// $comment_content .= '<div style="font-size:10px; color:#aaa; text-align:right;">'.$comment['comment_date']."</div></div>";
						if($comment['user_id'] == ucwords(strtolower($this->ident['user_id']))){
							$comment_content .= '<div class="msg-chat sent" ><strong>'.$comment['name'].' : </strong>'.$comment['comment'].'<br/>'.$comment['comment_date'];
							}else{
								$comment_content .= '<div class="msg-chat rcvd" ><strong>'.$comment['name'].' : </strong>'.$comment['comment'].'<br/>'.$comment['comment_date'];
							}
							if(!empty($comment['filename'])) $comment_content .='<a href="'.$this->baseUrl."/comments/".substr($comment['comment_date'],0,4)."/".$comment['filename'].'" target="_blank"><i class="fa fa-paperclip"></i> '.$comment['filename'].'</a>';
							$comment_content .= '<div class >'.$comment['comment_date_']."</div></div>";
					}
					$data[$i]['comment'] = $comment_content;
				}
				$i++;
	...
	function getkejadianbynameAction()
	{
		$params = $this->_getAllParams();
		
		if($params['template_id'] == 4) $issue_type = 15;
		if($params['template_id'] <= 5) $issue_type = 16;
		else $issue_type = 15;

		Zend_Loader::LoadClass('incidentClass', $this->modelDir);
		$incidentTable = new incidentClass();
		$incident = $incidentTable->getIncidentIdByNameTypeSites($params['category_id'], $params['kejadian_name'], $issue_type, $this->site_id);
		echo intval($incident[0]['kejadian_id']);
	}
	
	function getmodusbynameAction()
	{
		$params = $this->_getAllParams();
		Zend_Loader::LoadClass('modusClass', $this->modelDir);
		$modusTable = new modusClass();
		$modus = $modusTable->getModusIdByModusNameAndSites($params['category_id'], $params['modus_name'], $this->site_id, $params['kejadian_id']);
		
		echo intval($modus[0]['modus_id']);
	}

	function newexportissuestopdfAction() {
		$params = $this->_getAllParams();
		...
		$categories = $categoryClass->getCategories_2($params['id']);
		Zend_Loader::LoadClass('issueClass', $this->modelDir);
		$issueClass = new issueClass();
		...
		if(!empty($categories))
		{
			...
			foreach($categories as $cat)
			{
				$params['id'] = $params['id'];
				$params['id'] = $params['id'];
				//$params['id'] = $cat['categories_id'];
				$params['site_id'] = $this->site_id;
				//echo $params['category'];
				//exit();
				$issues = $issueClass->getIssues_2($params);
