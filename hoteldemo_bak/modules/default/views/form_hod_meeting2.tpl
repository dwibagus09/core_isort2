	  ...
	  <div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel">
			...
			<form class="form-label-left" action="/default/hod/save2" method="POST" onsubmit="$('body').mLoading();">
				...
				<div class="x_title">
					<h2 class="page-title">Coordination Meeting MoM</h2>
				...
				<div class="x_content">
					...
					<div style="margin-bottom:10px; clear:both;">
						<a class="add-topic" href="#topic-form"><input type="button" value="Add Project" style="width:150px;" class="form-btn"></a>
					</div>
					...
					?>
						...
						?>
						<tr>
						<td align="center"><?php echo $topic['category_name']; ?></td>
...
<form action="/default/hod/savetopic" id="topic-form" class="mfp-hide white-popup-block" method="post" enctype="multipart/form-data" >
	...
	<table id="add-topic" class="topic-form-table" width="100%">
		<tr>
			...
			<td align="center">
			    <select id="category-select2" name="department_id" class="form-control col-md-7 col-xs-12" required>
		...
		<tr>
			<td valign="top">Documents <a id="add-image"><i class="fa fa-plus-square"></i></a><td>
			<td align="left" valign="top" id="topic_image_list">
				<div id="list-images"></div>
				<ul class="hod_image">
					<li><input type="file" name="topic_image[]" style="display:inline-block; font-size:13px;"> <i class="fa fa-trash remove-image" style="cursor:pointer;" onclick="$(this).closest('li').remove();"></i></ul>
				</ul>
			</td>
		</tr>
	</table>
...
$(document).ready(function() {
		...
		callbacks: {
			open: function() {
				...
				$.ajax({
					...
					$("#category-select2").val(resp.department_id);
	...
	$('#add-image').on('click', function(event){
		$( "#topic_image_list .hod_image").append('<li><input type="file" name="topic_image[]" style="display:inline-block;font-size:13px;"> <i class="fa fa-trash remove-attendance" style="cursor:pointer;" onclick="$(this).closest(\'li\').remove();"></i></li>');
	});
