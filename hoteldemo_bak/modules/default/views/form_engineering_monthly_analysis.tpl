...
<?php 
if(empty($this->engineering['report_date'])) $cur_date = date("Y-m-d");
else {
	$cur_report_date = explode(" ",$this->engineering['report_date']);
  ...
  <div class="detail-report">
	...
	<div class="row">
	  <div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel">
		  <div class="x_title">
			<h2 class="pagetitle">Engineering Monthly Analytics <?php echo $this->monthYear; ?></h2>
		  ...
		  <div class="x_content">
			...
			<table id="perlengkapan-table" class="table">
			  ...
			  <tbody>
				...
				<tr>
					...
					<td align="center"><a href="#issue-monthly-form" class="action-btn" data-id="<?php echo $issue['issue_id']; ?>"><i class="fa fa-edit" ></i></a></td>
				</tr>
				<?php $i++; } ?>
			  </tbody>
			</table>
			...
			<form action="" id="issue-monthly-form" class="mfp-hide white-popup-block" >
			...
			<form class="form-horizontal form-label-left" action="/default/engineering/savemonthlyanalysis" method="POST" onSubmit="$('body').mLoading();">
...
$(document).ready(function() {
			...
			open: function() {
			  $.ajax({
					...
					$.ajax({
						...
						data: { kejadian_id : resp.kejadian_id, category_id : '6' }
					}).done(function(response) {
						$("#modus_id").empty();
					...
					$("#kejadian_id").change(function() {
						...
						{			
							...
							$.ajax({
								url: "/default/issue/getmodusbykejadianid",
								data: { kejadian_id : $( this ).val(), category_id: '6'  }
							}).done(function(response) {
								$("#modus_id").empty();
								var object = $.parseJSON(response);
								$.each(object, function (item, value) {
	...
	$('#issue-monthly-form').on('submit', function(event){
		...
		$.ajax({
			...
			success: function() {
				...
				<?php } ?>
				location.href="/default/engineering/addmonthlyanalysis"+idparam+"?"+Math.random()+"#issue"+issue_id;
			}
		});
	});
